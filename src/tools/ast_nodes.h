...
static an_decl_pat an_decl_pat_new_tuple(box name, an_tuple_pat tuple) {
    return (an_decl_pat){
        .tag = AN_DECL_PAT_TUPLE,
        .data.tuple.name = name,
        .data.tuple.tuple = tuple,
    };
}

static an_decl_pat an_decl_pat_new_name(bool tilda, box name) {
    DEBUG_ASSERT(box_data(name));

    return (an_decl_pat){
        .tag = AN_DECL_PAT_NAME,
        .data.name.tilda = tilda,
        .data.name.name = name,
    };
}

static an_decl_pat an_decl_pat_new_under() {
    return (an_decl_pat){
        .tag = AN_DECL_PAT_UNDER,
    };
}

static void an_decl_pat_drop(an_decl_pat *self) {
    switch (self->tag) {
...
static an_decl an_decl_new(an_decl_pat pat, ast typ, ast val) {
